...
Let's say that we just finished the reviewing our blueprints for the small house renovation project and that we are moving back to emacs to update our project.
...
- "Schedule carpenter" is still TODO - waiting for "Order windows" to complete
  ...
  In a real life scenario, we would automatically know what's the next part of the project without having to go back to the larger project view and having to face all the tasks to do.
...
When all tasks are done, the project is complete. You can archive it with ~M-x org-gtd-archive-completed-items~ during your weekly review, as proposed in the GTD book. This is also important to make sure your project list stays manageable in the long-term.
...
Look into your list of next tasks which of them can be done quickly and add an effort estimate to them by using the Org command ~org-clock-modify-effort-estimate~, bound by default to ~C-c C-x e~. You can simply write 5min or any other time that you think is necessary.

Once you have a few tasks with an effort estimate lower than 15min, try this in your ~*scratch*~ buffer:
...
- ~"15"~ , ~"15min"~ or ~"0:15"~ → 15 minutes
- ~"90"~ , ~"1:30"~ or ~"1h30min"~ → 1 hour 30 minutes
- ~"2:00"~ or ~"2h"~ → 2 hours
...
- Are important (priority A or B) (using ~org-priority~, bound by default to ~C-c ,~)
